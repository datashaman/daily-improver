...
The core scores every language-neutral candidate kind with an exhaustive category-specific weight table. Impact and confidence always increase a score; effort and change risk always reduce it. The relative weights reflect the category without encoding PHP or framework knowledge, and equal scores are resolved by stable candidate ID.
