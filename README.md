...
Builder path confinement rejects multiply hard-linked production and protected files in addition to symbolic-link and traversal escapes. The runner snapshots the disposable root, allowlisted parent chains, and original targets by device/inode identity; builder-created links and parent replacements fail closed. Approved regular-file bytes move through identity- and SHA-256-bound trusted staging. Protected inputs, the disposable target and parent chain, the staged file, and the unchanged source-checkout target and parent chain are revalidated immediately before each atomic import, so a post-check replacement cannot redirect an approved write.

Immediately before and after builder execution, the runner also captures an exact `builder-filesystem-state/v1` snapshot of the complete disposable copy and derives `builder-filesystem-change-set/v1`. Sorted entries describe regular files, directories, symbolic links, and unsupported filesystem types using bounded paths, modes, sizes, and SHA-256 identities without retaining file contents or symbolic-link targets. The change set exhaustively classifies additions, modifications, deletions, and type changes, including paths outside the production write allowlist. Capture uses runner-owned entry, path, file-byte, and symbolic-link bounds, streams regular-file hashing, repeats the walk to prove stability, and fails closed on unreadable, malformed, traversing, excessive, or changing state.

The complete change set is checked before a builder result or failure is processed and before any file can be imported. Every `builder-protected-inputs/v1` file must match its captured baseline SHA-256 identity, its expanded parents must be captured directories, and any addition, modification, deletion, or type change at or below those protected paths fails immediately. The decision retains only bounded paths, entry types, metadata, and hashes, never source contents; even a builder that lifts disposable-copy permissions and replaces a sealed input cannot export an approved production change.

Git and GitHub CLI entry points are always intercepted inside command-backed builders by a separate runner-owned layer. Direct, PATH-resolved, explicit-path, and shell-indirect calls fail before the real tool runs; builders cannot replace PATH or obtain an exception to commit, push, or open a pull request. This publication denial remains active even when network or dependency access is approved and does not depend on the disposable copy lacking Git metadata.

Each agent runs in an isolated worktree. Command-backed agents do not inherit the CLI process environment: the runner exposes only a validated absolute `PATH`, `DAILY_IMPROVER_AGENT_STAGE`, and the exact repository-contained `DAILY_IMPROVER_SPEC_PATH` through a non-login shell. Unsafe or extended runtime environments fail before execution, so test, analysis, manifest, control-plane, GitHub, and unrelated model credentials cannot cross into the builder process. Commands that require authentication must obtain it from an explicit runner-owned mechanism rather than ambient environment inheritance. Command-backed builders additionally default to `builder-network-policy/v1` outbound denial. The runner verifies the macOS sandbox or Linux network namespace against a live runner-owned socket before executing the builder; unavailable or ineffective isolation fails closed. Only an exact policy supplied directly by the trusted runner may approve outbound access—checkout configuration, specifications, commands, and model output cannot do so. Builders also default to `builder-dependency-installation-policy/v1` denial. A runner-owned shim layer blocks Composer, JavaScript, Python, Ruby, Rust, Go, JVM, and .NET package-manager entry points before the real executable runs; indirect PATH-resolved calls, explicit package-manager paths, and command-level PATH replacement fail closed. Only an exact constructor policy may approve package-manager execution, independently of the outbound-network decision. Every builder requires exact runner-owned `builder-resource-limits/v1` CPU, memory, disk-growth, combined-output, and wall-clock limits. Unsupported or malformed limits stop before execution; exhaustion terminates the complete process group, escalates to a forced kill, retains only bounded output, and reports the exhausted resource without builder output. The test agent must add proof appropriate to the versioned improvement intent, while the builder must implement the spec without touching sealed tests. Before builder invocation, protected scopes are removed from the specification-derived write allowlist and every remaining entry must be one exact bounded production file. Absolute, traversing, wildcard, duplicate, excessive, protected, non-regular, and symlink-crossing entries fail before the builder. The builder receives a disposable copy without Git metadata, and only allowlisted regular-file changes are imported back. In that copy, `builder-protected-inputs/v1` binds sealed manifest hashes and trusted configured scopes for tests, specifications, policies, workflows, and migrations. Missing, replaced, mutable, non-regular, or symlink-crossing inputs fail before invocation; accepted files and their parent directories are read-only and are rechecked before import. Defect regressions must fail behaviorally against the baseline. Refactor characterizations, performance measurements, and maintainability quality checks must pass at baseline, and every intent must pass independent post-change verification. Every generated test must write nonce-bound `generated-test-lifecycle-report/v1` to `DAILY_IMPROVER_TEST_LIFECYCLE_PATH` during three baseline and three post-change executions. The report identifies each generated test as executed, skipped, or disabled and supplies its assertion count and effective-tolerance hash. Missing, skipped, disabled, assertion-free, deleted, changed, weakened, or inconsistently executing tests fail closed; inconsistent outcomes or metrics quarantine the candidate before building or publishing. Retained lifecycle decisions contain bounded commands, outcomes, durations, metrics, and output hashes without raw output. When Pest or PHPUnit is detected, the PHP adapter additionally emits exact framework-specific quality evidence bound to those lifecycle paths, file hashes, and assertion counts. Pest inspection rejects focused, skipped, todo, assertion-free, malformed, oversized, unsupported, or unprovably bounded provider structures. PHPUnit inspection recognizes public convention, attribute, and docblock tests inside `TestCase` subclasses and rejects skipped/incomplete markers, per-method assertion gaps, malformed or unsupported discovery, and empty, external, dynamic, or missing providers. When Eris is also detected, the ordinary runner gate is followed by exact Eris inspection bound to the validated property proof, selected test, production target, approved invariant, and executed input count. Eris inspection requires `TestTrait`, supported static generator factories, direct `forAll(...)->then(...)` execution, a target invocation, and an invariant assertion, while rejecting dynamic or fake generators and iteration overrides. Accepted evidence retains only bounded semantic inputs, paths, hashes, counts, and exhaustive signal names. Property tests also receive runner-owned `DAILY_IMPROVER_PROPERTY_PROOF_PATH`, `DAILY_IMPROVER_PROPERTY_EXECUTION_NONCE`, `DAILY_IMPROVER_PROPERTY_TARGET`, and `DAILY_IMPROVER_PROPERTY_INVARIANTS` values during the test command. They must emit `property-test-execution-proof/v1` with 32–1,000 unique input digests and one target execution plus invariant check per input; invalid or mismatched proof stops before the builder. The runner then emits `test-implementation-inspection/v1`, binding the observed generated test to the selected target and approved invariant. Direct production-source inspection, long exact token copies, and long identifier-normalized structural copies stop before the builder, while retained evidence contains only bounded paths, hashes, metrics, and signal names. When evidence explicitly identifies the baseline as a known mutant, the specification carries an exact `known-mutation/v1` requirement. The trusted runner then records `known-mutation-execution-proof/v1`, binding the relevant generated test to that mutation, selected target, approved criterion, exact command, credible failing outcome, duration, and output hashes. A missing, malformed, survived, wrong-test, wrong-target, wrong-criterion, or non-behavioral result stops before the builder; raw source and command output are not retained.

The command-backed provider remains available for the local CLI. The first structured model provider accepts an injected transport and uses separate versioned `test-agent-request/v1`, `test-agent-response/v1`, `builder-request/v1`, and `builder-response/v1` contracts. Requests expose only the bounded semantic task, its exact `improvement-intent/v1` contract, repository language/framework context, explicit commands and conventions, and stage-specific repository-relative path permissions. They never serialize host repository paths or credentials. Missing, malformed, extended, unsupported, or inconsistent intent contracts fail before transport. Responses fail closed on unknown fields, unsupported versions, unbounded text or collections, unsafe or unauthorized changed-file claims, malformed commands, or invalid token, latency, and cost usage. Before transport, a `task-complexity-decision/v1` deterministically selects a lower- or higher-complexity test/build route from an exact `model-routing-policy/v1`. The score uses only validated file and changed-line limits plus acceptance-criteria, property-invariant, and evidence counts; it never reads task prose, repository source, host paths, credentials, or model rationale. Each of the four routes has a unique bounded ID and explicit provider/model target. An exact `model-endpoint-policy/v1` then assigns every route exactly once to a bounded endpoint ID with endpoint-neutral structured-protocol, supported-stage, ephemeral-authentication, and maximum-cost capabilities. Incomplete, unsupported, extended, duplicate, same-target, uncovered, or stage-incompatible configuration fails closed before transport, and response usage must match the selected route. Endpoint resolution stays inside the injected transport; URLs and provider-specific behavior do not enter core stage contracts. Structured transports require explicit test, builder, and aggregate daily cost budgets; an injectable ledger reserves cost before every attempt against all three limits and the specification cost ceiling, then accounts validated actual usage. Attempts without validated usage conservatively consume their full reservation. Only an explicitly classified transient transport failure may retry, using a bounded attempt schedule and injected timing; permanent transport failures, malformed responses, policy/path violations, and budget failures stop immediately. Every attempt also requires an injected `model-stage-credential/v1` credential scoped to the exact test or builder stage and repository/specification run, valid for no more than fifteen minutes. Invalid credentials stop before transport, and one secret cannot cross from test to build. Credential secrets remain ephemeral transport context and never enter endpoint metadata, requests, responses, trusted usage, untrusted rationale, routing decisions, attempt metadata, or errors. The versioned trusted `agent-usage/v4` artifact records the routing decision, sanitized attempt classifications, and per-attempt budget decisions without transport error messages. Summaries and implementation notes remain separate and are explicitly marked as untrusted model rationale.
...
3. `test` enforces three stable intent-specific baseline executions and seals generated regression, characterization, measurement, and quality tests plus `test-plan/v7` and `generated-test-lifecycle-decision/v1`; detected Pest or PHPUnit work also seals its exact framework-specific generated-test quality inspection, detected Eris property work seals proof-bound `eris-property-test-quality-inspection/v1`, property work additionally seals its validated `property-test-execution-proof/v1` and `test-implementation-inspection/v1` artifacts, and applicable known-mutant work seals `known-mutation-execution-proof/v1`.
...
6. `publish` re-resolves trusted main, records the unchanged identity, completes the active repository/day claim, and emits one publication request for the GitHub App to turn into a draft PR.

In the local proving loop, verifier inputs are captured before generated code runs and sealed before the builder. The exact expected commit SHA, specification and repository-configuration identities, protected-path policy, bounded commands, runner-owned `verifier-command-environment/v1` decision, sealed test-manifest identities, fixed runner artifact paths, and verification output destination form `verifier-execution-inputs/v2`. Verification revalidates every identity. Each repository command and post-change lifecycle attempt gets a separate non-login process with the sealed absolute PATH, new empty HOME/temp/XDG directories, and no inherited ambient variables; the runner probes that exact environment before repository code executes and removes its storage afterward. Builder response fields and implementation notes cannot change commands, policies, executable resolution, credentials, caches, baseline identity, sealed artifacts, or output routing. The resulting `verification-report/v1` is atomically written only after the independent checks run and binds the expected baseline and verifier-input SHA-256.

Verifier and publisher handoffs are authenticated with the runner-owned `DAILY_IMPROVER_MANIFEST_KEY`. Sealed inputs use exact `test-manifest/v2`; the verification report, post-change lifecycle decision, publication patch, completed daily decision, publication authorization, and exact `publication-request/v1` each have an `artifact-authentication/v1` sidecar. Each sidecar binds the repository-relative path, supported artifact schema, SHA-256, and bounded validity window with HMAC-SHA-256. Missing, malformed, extended, unsupported, stale, mismatched, non-regular, symlinked, traversing, or ambiguously addressed artifacts fail before verifier commands, claim completion, request emission, staging, or commit creation.

Verification runs in a new runner-owned clone checked out detached at that sealed SHA, never in the test-generation or builder-import worktree. The runner rejects a missing, advanced, mismatched, non-commit, or ambiguous baseline before repository commands run. It materializes only exact approved production file states, verifier-relevant manifest-bound tests and artifacts, and the signed manifest; builder-only files, caches, runtime reports, copied builder Git state, build-agent artifacts, and model rationale remain absent. The approved diff, manifest identities, semantic gates, configured commands, and three post-change lifecycle attempts are evaluated in that clean checkout. Only the verification report and lifecycle decision return to the publication worktree.

Immediately before `publish` completes the daily claim or emits a draft-PR request, it resolves the runner-supplied trusted main reference again and requires that one commit to equal the baseline bound into `verification-report/v1`. Missing, advanced, rewound, non-commit, mismatched, or ambiguous main state fails closed. Successful publication writes `publication-authorization/v1` with only the expected and checked commit SHAs, verifier-input SHA-256, authorized outcome, and decision time. The standalone `publish` command checks `DAILY_IMPROVER_BASE_REF`, defaulting to `origin/main`; the complete local runner checks the original source checkout's `HEAD`.

The complete local runner never commits the builder-import worktree. After successful fresh verification and lifecycle checks, it creates another clean checkout at the verified SHA and materializes only the exact production states, verifier-relevant sealed artifacts, signed manifest, verification report, and lifecycle decision. `verified-publication-patch/v1` binds each regular file by path, SHA-256, and mode, represents approved deletions explicitly, and separately binds the verification report and lifecycle identities. Any extra, missing, changed, non-regular, symlinked, traversing, or overlapping input stops before staging. Publication then stages only that bounded set plus the trusted authorization, request, and daily-claim transition; builder-only files, caches, Git state, build-agent artifacts, model rationale, and other workspace changes cannot enter the branch commit.
...
The end-to-end fixture proves the intended contract: a generated property test executes 510 unique inputs, binds every check to the selected MoneyAllocator target and approved sum invariant, records an observed baseline violation, and fails against the morning baseline. A bounded builder change then makes it pass, protected proof artifacts remain unchanged, semantic safety checks pass, and the verified commit is retained on an `ai/daily/<date>-<description>` branch.
