...
In the local proving loop, verifier inputs are captured before generated code runs and sealed before the builder. The exact expected commit SHA, specification and repository-configuration identities, protected-path policy, bounded commands, runner-owned PATH, sealed test-manifest identities, fixed runner artifact paths, and verification output destination form `verifier-execution-inputs/v1`. Verification revalidates every identity and runs commands through absolute `/bin/sh` with no ambient credentials. Builder response fields and implementation notes cannot change commands, policies, executable resolution, baseline identity, sealed artifacts, or output routing. The resulting `verification-report/v1` is atomically written only after the independent checks run and binds the expected baseline and verifier-input SHA-256.
