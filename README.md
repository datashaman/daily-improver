...
Committed `structured-provider-replay/v1` fixtures under `test/fixtures/model-provider-replay/` replay both structured stages without a live endpoint or permanent credential. The builder replay includes a classified transient failure and verifies deterministic retry timing, fresh credential acquisition, conservative failure accounting, validated response usage, and removal of transport error text from provider results.
