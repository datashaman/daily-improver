...
export async function captureVerifierMutationState(root: string, base: string, runner: CommandRunner): Promise<string> {
...
export async function assertVerifierMutationStateUnchanged(
  root: string,
  base: string,
  expectedState: string,
  runner: CommandRunner,
): Promise<void> {
  if (!/^[a-f0-9]{64}$/u.test(expectedState) || await captureVerifierMutationState(root, base, runner) !== expectedState) {
    throw new Error("Targeted mutation execution changed the fresh verifier checkout.");
  }
}
