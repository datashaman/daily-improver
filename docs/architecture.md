...
Committed `structured-provider-replay/v1` fixtures exercise the test and builder protocols without repository access, a live model endpoint, or permanent credentials. They replay through the injected transport, credential clock and source, retry timing, and cost ledger, and pin versioned requests, validated responses, usage, attempts, and budget decisions. A transient builder failure proves conservative reservation accounting and retry sanitization: transport error text is fixture input only and cannot enter the trusted or untrusted provider result.
