...
Every input removed before autonomous selection also produces one bounded `candidate-exclusion/v1` record at its first failed gate. The exhaustive reason set distinguishes malformed scope, evidence, scoring, semantic deduplication, and oversized autonomous scope. Records are sorted deterministically and retain only a bounded candidate reference, an optional valid category, and the retained candidate reference for deduplication; invalid IDs become hashes. They never retain evidence, provenance, rationale, titles, targets, or source paths. The version 3 analysis artifact and persisted planning runs carry these records, including rejected runs where no candidate survives.
